...
#define immJ() do { *imm = SEXT((BITS(i, 30, 21) << 1) + (BITS(i, 20, 20) << 11) \
  + (BITS(i, 19, 12) << 12) + (BITS(i, 31, 31) << 20), 21); } while(0)
#define immB() do { *imm = SEXT((BITS(i, 7, 7) << 11) + (BITS(i, 11, 8) << 1) \
  + (BITS(i, 30, 25) << 5) + (BITS(i, 31, 31) << 12), 13);} while(0) 
...
#define ADD_ELF_LOG_CALL   is_func_addr(s->pc, s->dnpc)
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(rd) = (uint64_t)Mr(src1 + imm, 4));
  ...
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(rd) = ((int64_t)(src1)) >> SHAMT);
  INSTPAT("010000 ?????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT(((int32_t)(src1)) >> SHAMT, 32));
